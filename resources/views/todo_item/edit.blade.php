<x-app-layout>
	...
	<div class="block p-6 rounded-lg shadow-lg bg-white max-w-sm mx-auto mt-5">
		<form method="post" action="{{route('todo_item.update', $todo_item)}}">
			@csrf
			@method('patch')
			...
			<div class="form-group mb-6">
				...
				<input type="text" class="form-control
					...
					name="item_name" placeholder="Enter your task" value="{{old('item_name', $todo_item->item_name)}}">
			</div>
		 	<div class="form-group mb-6">
		  	...
		  	<div class="form-group form-check mb-6">
				...
				<input type="date" class="form-control
					...
					m-0
					focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" 
					name="expire_date" value="{{old('expire_date', $todo_item->expire_date)}}">
			</div>
		  	<div class="form-group form-check mb-6">
				<input type="checkbox"
			  		class="form-check-input appearance-none h-4 w-4 border border-gray-300 rounded-sm bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer"
			  		name="done" @if (isset($todo_item->finished_date)) checked @endif>
